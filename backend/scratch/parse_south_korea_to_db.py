...
FILE_MAP = {
    'AJOU UNIVERSITY South Korea.docx': ('Suwon', 'Gyeonggi Province'),
    'CHONNAM NATIONAL UNIVERSITY South Korea.docx': ('Gwangju', 'South Jeolla'),
    'EWHA WOMANS UNIVERSITY South Korea.docx': ('Seoul', 'Seoul'),
    'GACHON UNIVERSITY South Korea.docx': ('Seongnam', 'Gyeonggi Province'),
    'HANSEO UNIVERSITY FOR INTERNATIONAL  South KoreaSTUDENTS.docx': ('Seosan', 'South Chungcheong'),
    'HANYANG UNIVERSITY SOUTH KOREA.docx': ('Seoul', 'Seoul'),
    'JEONBUK NATIONAL UNIVERSITY South Korea.docx': ('Jeonju', 'North Jeolla'),
    'KAIST South Korea.docx': ('Daejeon', 'Daejeon'),
    'KOREA UNIVERSITYSOUTH KOREA.docx': ('Seoul', 'Seoul'),
    'KYUNG HEE UNIVERSITY SOUTH KOREA.docx': ('Seoul', 'Seoul'),
    'KYUNGPOOK NATIONAL UNIVERSITY South Korea.docx': ('Daegu', 'Daegu'),
    'POSTECH SOUTH KOREA.docx': ('Pohang', 'North Gyeongsang'),
    'PUSAN NATIONAL UNIVERSITY South Korea.docx': ('Busan', 'Busan'),
    'SEJONG UNIVERSITY SOUTH KOREA.docx': ('Seoul', 'Seoul'),
    'SEOUL NATIONAL UNIVERSITY South Korea.docx': ('Seoul', 'Seoul'),
    'SOGANG UNIVERSITY South Korea.docx': ('Seoul', 'Seoul'),
    'SOLBRIDGE INTERNATIONAL BUSINESS SCHOOL South Korea.docx': ('Daejeon', 'Daejeon'),
    'SOOKMYUNG WOMEN South Korea.docx': ('Seoul', 'Seoul'),
    'SUNGKYUNKWAN UNIVERSITY SOUTH KOREA.docx': ('Seoul', 'Seoul'),
    'YONSEI UNIVERSITY South Korea.docx': ('Seoul', 'Seoul')
}
...
def parse_fee_to_cad(text_line):
    ...
    if any(word in line_lower for word in ["no tuition", "free", "0 krw", "0 usd", "no fees"]):
        return 0
    ...
    # 1) Try KRW first
    krw_match = re.findall(r'krw\s*([0-9,]+)', line_lower)
    if not krw_match:
        krw_match = re.findall(r'([0-9,]+)\s*(?:krw|won)', line_lower)
    if krw_match:
        try:
            val = int(krw_match[0].replace(',', '').strip())
            if val > 0:
                # 1 KRW = 0.0010 CAD
                return int(val * semester_mul * 0.0010)
        except ValueError:
            pass
    ...
    for nc in num_candidates:
        ...
        if val_clean.isdigit():
            ...
            elif 500000 <= val_int <= 25000000: # likely KRW
                valid_nums.append((val_int, 0.0010))
    if valid_nums:
...
def parse_docx_file(path, filename):
    ...
    # Special cleanup for titles ending with "South Korea" or "For International Students"
    clean_name = re.sub(r'\s*(?:south korea|for international students)\s*$', '', clean_name, flags=re.IGNORECASE).strip()

    city, state = FILE_MAP.get(filename, ('Unknown City', 'South Korea Region'))

    # Avoid duplicate name constraints by appending suffix if not already containing country name
    if "korea" not in clean_name.lower():
        clean_name = f"{clean_name} (South Korea)"
    ...
    inst = {
        ...
        "intakes": ["Semester 1 (March)", "Semester 2 (September)"],
        "deadlines": {},
        "eligibility": {
            "undergraduate": {},
            "postgraduate": {}
    ...
    for p in course_paras:
        ...
        if prog_name:
            ...
            current_prog = {
                ...
                "name": prog_name,
                "faculty": "",
                "duration_years": 4.0 if level == "UG" else 2.0, # standard defaults in Korea
                "annual_fee_cad": 0,
                "total_fee_cad": 0,
...
def main():
    ...
    print("Clearing existing South Korea institutions and programs...")
    sk_insts = list(db.institutions.find({"country": "South Korea"}))
    sk_ids = [inst["_id"] for inst in sk_insts]
    
    if sk_ids:
        del_progs = db.programs.delete_many({"institution_id": {"$in": sk_ids}})
        print(f"Deleted {del_progs.deleted_count} existing South Korea programs.")
        
    del_insts = db.institutions.delete_many({"country": "South Korea"})
    print(f"Deleted {del_insts.deleted_count} existing South Korea institutions.")
    
    dir_path = '/Users/mehulmanoharsah/Downloads/SOUTH KOREA '
