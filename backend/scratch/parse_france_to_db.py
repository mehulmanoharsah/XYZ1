...
def get_location_for_file(filename):
    ...
    if 'audencia' in fn_lower: return ('Nantes', 'Pays de la Loire')
    if 'coll' in fn_lower: return ('Paris', 'Île-de-France')
    if 'edhec' in fn_lower: return ('Lille', 'Hauts-de-France')
    if 'essec' in fn_lower: return ('Cergy-Pontoise', 'Île-de-France')
    if 'ponts' in fn_lower: return ('Champs-sur-Marne', 'Île-de-France')
    if 'hec' in fn_lower: return ('Jouy-en-Josas', 'Île-de-France')
    if 'insead' in fn_lower: return ('Fontainebleau', 'Île-de-France')
    if 'polytechnique' in fn_lower: return ('Palaiseau', 'Île-de-France')
    if 'kedge' in fn_lower: return ('Bordeaux', 'Nouvelle-Aquitaine')
    if 'neoma' in fn_lower: return ('Reims', 'Grand Est')
    if 'rennes' in fn_lower: return ('Rennes', 'Brittany')
    if 'schiller' in fn_lower: return ('Paris', 'Île-de-France')
    if 'sciences po' in fn_lower: return ('Paris', 'Île-de-France')
    if 'skema' in fn_lower: return ('Lille', 'Hauts-de-France')
    if 'sorbonne' in fn_lower: return ('Paris', 'Île-de-France')
    if 'tbs' in fn_lower: return ('Toulouse', 'Occitanie')
    if 'grenoble' in fn_lower: return ('Grenoble', 'Auvergne-Rhône-Alpes')
    if 'cite' in fn_lower or 'cité' in fn_lower: return ('Paris', 'Île-de-France')
    if 'paris' in fn_lower and 'psl' in fn_lower: return ('Paris', 'Île-de-France')
    if 'psl' in fn_lower: return ('Paris', 'Île-de-France')
    if 'paris' in fn_lower: return ('Orsay', 'Île-de-France')
...
def parse_docx_file(path, filename):
    ...
    inst = {
        ...
        "application_fee_cad": int(100 * 1.50), # Default fallback France fee (approx €100)
    ...
    for p in step1_paras:
        if "Scholarships" in p or "Financial Aid" in p:
            ...
            continue
        if "Career Outcomes" in p or "Internships" in p or "Jobs" in p or "Work Allowance" in p:
            scholarship_block = False
    ...
    for p in course_paras:
        ...
        if prog_name:
            ...
            current_prog = {
                ...
                "name": prog_name,
                "faculty": "",
                "duration_years": 3 if level == "UG" else 2, # Standard France durations (3-4 years UG, 1-2 years PG)
                "annual_fee_cad": 0,
                "total_fee_cad": 0,
...
def main():
    ...
    print("Clearing existing France institutions and programs...")
    fr_insts = list(db.institutions.find({"country": "France"}))
    fr_ids = [inst["_id"] for inst in fr_insts]
    
    if fr_ids:
        del_progs = db.programs.delete_many({"institution_id": {"$in": fr_ids}})
        print(f"Deleted {del_progs.deleted_count} existing France programs.")
        
    del_insts = db.institutions.delete_many({"country": "France"})
    print(f"Deleted {del_insts.deleted_count} existing France institutions.")
    
    dir_path = '/Users/mehulmanoharsah/Downloads/FRANCE '
    ...
    print(f"Seeding completed. Inserted {inst_count} new France universities and {prog_count} new programs.")
