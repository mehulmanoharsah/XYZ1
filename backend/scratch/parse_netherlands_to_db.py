...
def get_location_for_file(filename):
    fn_lower = filename.lower()
    if 'avans' in fn_lower: return ('Breda', 'North Brabant')
    if 'breda' in fn_lower: return ('Breda', 'North Brabant')
    if 'erasmus' in fn_lower: return ('Rotterdam', 'South Holland')
    if 'fontys' in fn_lower: return ('Eindhoven', 'North Brabant')
    if 'hanze' in fn_lower: return ('Groningen', 'Groningen')
    if 'hotelschool' in fn_lower: return ('The Hague', 'South Holland')
    if 'leiden' in fn_lower: return ('Leiden', 'South Holland')
    if 'maastricht' in fn_lower: return ('Maastricht', 'Limburg')
    if 'radboud' in fn_lower: return ('Nijmegen', 'Gelderland')
    if 'saxion' in fn_lower: return ('Enschede', 'Overijssel')
    if 'the hague' in fn_lower: return ('The Hague', 'South Holland')
    if 'tilburg' in fn_lower: return ('Tilburg', 'North Brabant')
    if 'delft' in fn_lower: return ('Delft', 'South Holland')
    if 'eindhoven' in fn_lower: return ('Eindhoven', 'North Brabant')
    if 'amsterdam' in fn_lower: return ('Amsterdam', 'North Holland')
    if 'groningen' in fn_lower: return ('Groningen', 'Groningen')
    if 'twente' in fn_lower: return ('Enschede', 'Overijssel')
    if 'utrecht' in fn_lower: return ('Utrecht', 'Utrecht')
    if 'wageningen' in fn_lower: return ('Wageningen', 'Gelderland')
    return ('Amsterdam', 'North Holland')
...
def parse_fee_to_cad(text_line):
    ...
    if not numbers:
        # try simple numbers before or after EUR/euros/euro
...
def parse_docx_file(path, filename):
    ...
    # Clean country and guide keywords from title
    clean_name = re.sub(r'\s+NETHERLAND.*', '', clean_name, flags=re.IGNORECASE).strip()
    clean_name = re.sub(r'\s+NETEHERLAND.*', '', clean_name, flags=re.IGNORECASE).strip()
    clean_name = re.sub(r'\s+NETEHRLAND.*', '', clean_name, flags=re.IGNORECASE).strip()
    
    city, state = get_location_for_file(filename)
    ...
    inst = {
        ...
        "application_fee_cad": int(100 * 1.50), # Default fallback Netherlands fee (€100)
    ...
    for p in step1_paras:
        ...
        if "portal:" in p.lower() or "application portal" in p.lower() or "application portals" in p.lower():
            parts = p.split(":", 1)
            if len(parts) > 1:
                inst["application_portal"] = clean_text(parts[1])

    # Smart URL fallback for website
    if not inst["website"]:
        for p in step1_paras:
    ...
    for p in step1_paras:
        p_lower = p.lower()
        if "application fee" in p_lower or "application charge" in p_lower:
            parts = p.split(":", 1)
            if len(parts) > 1:
    ...
    for p in step1_paras:
        if "Scholarships" in p or "Financial Aid" in p or "Scholarship Names" in p:
            scholarship_block = True
            continue
        if "Career Outcomes" in p or "Internships" in p or "Jobs" in p or "Work Allowance" in p or "Part-time Work" in p:
            scholarship_block = False
    ...
    for p in step1_paras:
        ...
        if "clubs" in p_lower or "societies" in p_lower or "organizations" in p_lower:
    ...
    for p in course_paras:
        ...
        if prog_name:
            if ":" in prog_name:
                parts_col = prog_name.split(":", 1)
                if len(parts_col[0].strip()) < 60:
                    prog_name = parts_col[0].strip()
            ...
            current_prog = {
                ...
                "name": prog_name,
                "faculty": "",
                "duration_years": 3 if level == "UG" else 2, # Standard Netherlands durations (3 years UG, 1.5 - 2 years PG)
                "annual_fee_cad": 0,
                "total_fee_cad": 0,
                "min_class12_percent": 70, # Standard default
                "required_subjects": [],
                "entry_roles": [],
                "features": []
            }
        ...
        if current_prog:
            if "school name:" in p_lower or "department of" in p_lower or "college of" in p_lower or "department:" in p_lower or "department name:" in p_lower or "school:" in p_lower:
                parts = p.split(":", 1)
                current_prog["faculty"] = clean_text(parts[1]) if len(parts) > 1 else p_clean
            elif "total:" in p_lower or "duration" in p_lower or "total years" in p_lower or "duration:" in p_lower or "total duration:" in p_lower:
                duration_match = re.search(r'([0-9\.]+)\s*years?', p_clean)
                if not duration_match:
                    duration_match = re.search(r'([0-9]+)\s*months?', p_clean)
                    if duration_match:
                        current_prog["duration_years"] = float(duration_match.group(1)) / 12.0
                if duration_match and not current_prog.get("duration_parsed"):
                    if "month" not in duration_match.group(0):
                        current_prog["duration_years"] = float(duration_match.group(1))
                    current_prog["duration_parsed"] = True
            ...
            elif "minimum 12th" in p_lower or "class 12" in p_lower or "bachelor’s degree requirement" in p_lower or "bachelor's degree requirement" in p_lower or "eligibility" in p_lower or "minimum gpa" in p_lower:
            ...
            elif "typical job roles" in p_lower or "entry-level job roles" in p_lower or "roles:" in p_lower or "entry-level job roles:" in p_lower or "typical job roles:" in p_lower:
...
def main():
    ...
    print("Clearing existing Netherlands institutions and programs...")
    nl_insts = list(db.institutions.find({"country": "Netherlands"}))
    nl_ids = [inst["_id"] for inst in nl_insts]
    
    if nl_ids:
        del_progs = db.programs.delete_many({"institution_id": {"$in": nl_ids}})
        print(f"Deleted {del_progs.deleted_count} existing Netherlands programs.")
        
    del_insts = db.institutions.delete_many({"country": "Netherlands"})
    print(f"Deleted {del_insts.deleted_count} existing Netherlands institutions.")
    
    dir_path = '/Users/mehulmanoharsah/Downloads/NETHERLAND'
    ...
    print(f"Seeding completed. Inserted {inst_count} new Netherlands universities and {prog_count} new programs.")
