...
def parse_fee_to_cad(text_line):
    ...
    if any(word in line_lower for word in ["no tuition", "free", "s$0", "0 sgd", "no fees"]):
        return 0
    ...
    # Match SGD or S$
    numbers = re.findall(r's\$\s*([0-9,]+)', line_lower)
    if not numbers:
        numbers = re.findall(r'sgd\s*([0-9,]+)', line_lower)
    if not numbers:
        numbers = re.findall(r'([0-9,]+)\s*(?:sgd|s\$|dollars)', line_lower)
    if not numbers:
    ...
    if numbers:
        try:
            ...
            if val_str:
                val = int(val_str)
                if val > 0:
                    annual_sgd = val * semester_mul
                    # Convert SGD to CAD (1 SGD = 1.00 CAD)
                    return int(annual_sgd * 1.00)
...
def parse_docx_file(path, filename):
    ...
    if "singapore" not in clean_name.lower():
    ...
    inst = {
        ...
        "intakes": ["Semester 1 (August)", "Semester 2 (January)"],
        "deadlines": {},
        "eligibility": {
            "undergraduate": {},
            "postgraduate": {}
    ...
    for p in course_paras:
        ...
        if prog_name:
            ...
            current_prog = {
                ...
                "name": prog_name,
                "faculty": "",
                "duration_years": 3.0 if level == "UG" else 1.5, # standard defaults in Singapore
                "annual_fee_cad": 0,
                "total_fee_cad": 0,
...
def main():
    ...
    print("Clearing existing Singapore institutions and programs...")
    sg_insts = list(db.institutions.find({"country": "Singapore"}))
    sg_ids = [inst["_id"] for inst in sg_insts]
    
    if sg_ids:
        del_progs = db.programs.delete_many({"institution_id": {"$in": sg_ids}})
        print(f"Deleted {del_progs.deleted_count} existing Singapore programs.")
        
    del_insts = db.institutions.delete_many({"country": "Singapore"})
    print(f"Deleted {del_insts.deleted_count} existing Singapore institutions.")
    
    dir_path = '/Users/mehulmanoharsah/Downloads/SINGAPORE'
    ...
    print(f"Seeding completed. Inserted {inst_count} new Singapore universities and {prog_count} new programs.")
