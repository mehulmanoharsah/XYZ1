...
def main():
    ...
    institutions = list(db.institutions.find({"country": "Germany"}))
    print(f"Found {len(institutions)} German institutions to dump.")
    
    js_content = []
    js_content.append("// ============================================================")
    js_content.append("// GERMAN UNIVERSITIES & COLLEGES — MONGODB SEED SCRIPT")
    js_content.append("// 20 Institutions | UG & PG Programs | 2025-26")
    js_content.append("// ============================================================")
    js_content.append("// Usage: mongosh canadian_institutions germany_universities_seed.js")
    js_content.append("// ============================================================\n")
    ...
    # We will not drop the database or collections, but let's delete existing Germany records to avoid duplicates when running this seed file.
    js_content.append("// Delete existing German data to prevent duplicates")
    js_content.append("db.institutions.deleteMany({ country: \"Germany\" });")
    ...
    js_content.append("print(\"Germany Seeding Completed!\");")
    
    output_path = "/Users/mehulmanoharsah/Downloads/university-portal 3/germany_universities_seed.js"
