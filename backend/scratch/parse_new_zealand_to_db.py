...
FILE_MAP = {
    'ARA INSTITUTE OF CANTERBURY NEW ZEALAND.docx': ('Christchurch', 'Canterbury'),
    'AUCKLAND UNIVERSITY OF TECHNOLOGY NEW ZEALAND.docx': ('Auckland', 'Auckland'),
    'EASTERN INSTITUTE OF TECHNOLOGY NEW ZEALAND.docx': ('Napier', "Hawke's Bay"),
    'LINCOLN UNIVERSITY  NEW ZEALAND.docx': ('Lincoln', 'Canterbury'),
    'MANUKAU INSTITUTE OF TECHNOLOGY NEW ZEALAND.docx': ('Auckland', 'Auckland'),
    'MASSEY UNIVERSITY NEW ZELAND.docx': ('Palmerston North', 'Manawatu-Wanganui'),
    'NMIT NEW ZEALAND.docx': ('Nelson', 'Nelson'),
    'NORTHTEC NEW ZEALAND.docx': ('Whangarei', 'Northland'),
    'OTAGO POLYTECHNIC NEW ZEALAND.docx': ('Dunedin', 'Otago'),
    'SOUTHERN INSTITUTE OF TECHNOLOGY NEW ZEALAND.docx': ('Invercargill', 'Southland'),
    'TOI OHOMAI INSTITUTE OF TECHNOLOGY. NEW ZEALAND.docx': ('Rotorua', 'Bay of Plenty'),
    'UNITEC INSTITUTE OF TECHNOLOGY NEW ZEALAND.docx': ('Auckland', 'Auckland'),
    'UNIVERSITY OF CANTERBURY NEW ZELAND.docx': ('Christchurch', 'Canterbury'),
    'UNIVERSITY OF OTAGO NEW ZELAND.docx': ('Dunedin', 'Otago'),
    'UNIVERSITY OF WAIKATO NEW ZEALAND.docx': ('Hamilton', 'Waikato'),
    'VICTORIA UNIVERSITY OF WELLINGTON NEW ZELAND.docx': ('Wellington', 'Wellington'),
    'WAIKATO INSTITUTE OF TECHNOLOGY NEW ZEALAND.docx': ('Hamilton', 'Waikato'),
    'WELLINGTON INSTITUTE OF TECHNOLOGY NEW ZEALAND.docx': ('Wellington', 'Wellington'),
    'WHITIREIA NEW ZEALAND.docx': ('Porirua', 'Wellington')
}
...
def parse_fee_to_cad(text_line):
    ...
    # Match NZ$ or NZD$ or $
    numbers = re.findall(r'nz\$\s*([0-9,]+)', line_lower)
    if not numbers:
        numbers = re.findall(r'nzd\s*([0-9,]+)', line_lower)
    if not numbers:
        numbers = re.findall(r'\$\s*([0-9,]+)', text_line)
    if not numbers:
        numbers = re.findall(r'([0-9,]+)\s*(?:nzd|nz dollars|dollars)', line_lower)
    if not numbers:
    ...
    if numbers:
        ...
        if val > 0:
            ...
            if "semester" in line_lower and "year" not in line_lower:
                semester_mul = 2
            annual_nzd = val * semester_mul
            # Convert NZD to CAD (1 NZD = 0.82 CAD)
            return int(annual_nzd * 0.82)
    return 0
...
def parse_docx_file(path, filename):
    ...
    # Remove comma if it says "ARA INSTITUTE OF CANTERBURY, NEW ZEALAND"
    ...
    city, state = FILE_MAP.get(filename, ('Unknown City', 'New Zealand Region'))

    inst = {
        "name": clean_name,
        "abbreviation": abbr,
        "type": "university" if "university" in clean_name.lower() else "college",
        "city": city,
        "province": state,
        "country": "New Zealand",
        "campuses": [city],
        ...
        "application_fee_cad": 0, # Fallback default
    ...
    for p in step1_paras:
        ...
        if "email:" in p_lower:
            ...
            if len(parts) > 1:
                ...
                # Split multiple emails if necessary
    ...
    for p in course_paras:
        ...
        if prog_name:
            ...
            current_prog = {
                ...
                "name": prog_name,
                "faculty": "",
                "duration_years": 3.0 if level == "UG" else 2.0, # standard defaults
                "annual_fee_cad": 0,
                "total_fee_cad": 0,
...
def main():
    ...
    print("Clearing existing New Zealand institutions and programs...")
    nz_insts = list(db.institutions.find({"country": "New Zealand"}))
    nz_ids = [inst["_id"] for inst in nz_insts]
    
    if nz_ids:
        del_progs = db.programs.delete_many({"institution_id": {"$in": nz_ids}})
        print(f"Deleted {del_progs.deleted_count} existing New Zealand programs.")
        
    del_insts = db.institutions.delete_many({"country": "New Zealand"})
    print(f"Deleted {del_insts.deleted_count} existing New Zealand institutions.")
    
    dir_path = '/Users/mehulmanoharsah/Downloads/NEW ZELAND '
    ...
    print(f"Seeding completed. Inserted {inst_count} new New Zealand universities and {prog_count} new programs.")
